...
class Model:
    def __init__(self, input_user, input_item, output, num_users, num_items, rating_matrix, layers, batch_len, user_recent, attention_type):
        ...
        self.user_recent = user_recent # None*recent_len
    ...
    def predict(self):
        ...
        def attention_self_item(x, variable_scope):
        ...
        def attention_self(x, variable_scope):
            with tf.variable_scope(variable_scope):
                ...
                b_a0 = tf.get_variable('b_a0', [64, ])
                # x = tf.nn.relu(tf.matmul(x, W_a0) + b_a0)
                x = tf.nn.relu(tf.einsum('blw,wp->blp', x, W_a0) + b_a0)
                W_a1 = tf.get_variable('W_a1', [64, 1])
                b_a1 = tf.get_variable('b_a1', [1, ])
                # x = tf.matmul(x, W_a1) + b_a1
                x = tf.einsum('blw,wp->blp', x, W_a1) + b_a1
                return x

        def attention_cocat(x, item, variable_scope, func='concat'):
            with tf.variable_scope(variable_scope):
                if func == 'concat':
                    x = tf.concat([x, item], axis=2)
                    W_a0 = tf.get_variable('W_a0', [x.get_shape().as_list()[-1], 64])
                    b_a0 = tf.get_variable('b_a0', [64, ])
                    # tf.einsum('blp,blw->wp', x, W_a0)
                    x = tf.nn.relu(tf.einsum('blw,wp->blp', x, W_a0) + b_a0)
                    # x = tf.nn.relu(tf.matmul(x, W_a0) + b_a0)
                    W_a1 = tf.get_variable('W_a1', [64, 1])
                    b_a1 = tf.get_variable('b_a1', [1, ])
                    ...
                    x = x * item
                    x = tf.reduce_sum(x, axis=2, keep_dims=True)
                    return x
                else:
        ...
        recent_len = self.user_recent.get_shape().as_list()[1]
        ...
        user_recent_item_embedding = tf.nn.embedding_lookup(embedding_items, self.user_recent) #None*recent_len*embed_size
        assert user_recent_item_embedding.get_shape().as_list() == [batch_len, recent_len, self.embedding_size]

        # attention_type = "self"
        with tf.variable_scope("user_att_on_item"):
            # N = 10
            if self.attention_type == "self":
                att_embedding_items = attention_self(user_recent_item_embedding, "att_embed_items")
            else:
                # (batch_len*num_items, embedding_size)
                att_embedding = item_embedding
                att_embedding_rep = tf.expand_dims(att_embedding,axis=1)
                att_embedding_rep = tf.tile(att_embedding_rep, tf.constant([1,recent_len,1]))
                assert att_embedding_rep.get_shape().as_list() == [batch_len, recent_len, self.embedding_size]
                att_embedding_items = attention_cocat(
                        user_recent_item_embedding,
                        att_embedding_rep,
                        'att_embed_items',
                        func=self.attention_type)
            assert att_embedding_items.get_shape().as_list() == [batch_len, recent_len, 1]
            fenmu = tf.reduce_sum(tf.exp(att_embedding_items),axis=1,keep_dims=True) # batch_len*1*1
            fenzi = tf.exp(att_embedding_items) # batch_len*recent_len*1
            user_score_soft = fenzi/fenmu # batch_len*recent_len*1
            user_att_embedding = user_score_soft * user_recent_item_embedding
            user_att_embedding = tf.reduce_sum(user_att_embedding,axis=1)
        ...
        with tf.variable_scope("item_att_on_user"):
            ...
            att_embedding_users = tf.reduce_sum(attention_self_item(embedding_users, "att_embed_users"), axis=1)
            # else:
