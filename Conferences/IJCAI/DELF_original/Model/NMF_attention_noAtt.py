...
class Model:
    ...
    def predict(self):
        ...
        user_att_embedding = tf.expand_dims(tf.reciprocal(tf.sqrt(user_ratings_nonzero + 1)),
                                             axis=1) * tf.matmul(user_ratings, embedding_items)
        item_att_embedding = tf.expand_dims(tf.reciprocal(tf.sqrt(item_ratings_nonzero + 1)),
