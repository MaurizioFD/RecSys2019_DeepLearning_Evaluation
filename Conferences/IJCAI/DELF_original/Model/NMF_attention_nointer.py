    ...
    def predict(self):
        ...
        def mlp(embed_1, embed_2, variable_scope):
            with tf.variable_scope(variable_scope):
                ...
                y = tf.contrib.layers.fully_connected(y, self.layers[1])
                y = tf.contrib.layers.fully_connected(y, self.layers[2])
                # y = tf.contrib.layers.fully_connected(y, 1, tf.identity)
                return y

        user_embedding = user_embedding_mlp
        item_embedding = item_embedding_mlp
        x = tf.concat([user_embedding, user_att_embedding, item_embedding, item_att_embedding], axis=1)
        #
        ...
        x = tf.contrib.layers.fully_connected(x, self.layers[1]*4)
        x = tf.contrib.layers.fully_connected(x, self.layers[2]*4)
        x = tf.contrib.layers.fully_connected(x, self.predictor_size)
        x = tf.contrib.layers.fully_connected(x, 1, tf.identity)
        # x = ui_y * 0.25 + uni_y * 0.25 + uin_y * 0
        # x = tf.contrib.layers.fully_connected(x, self.predictor_size).25 + unin_y * 0.25
