...
class Model:
    ...
    def predict(self):
        ...
        # user_gating: batch_len*1
        def GRU(h, x, variable_scope):
            with tf.variable_scope(variable_scope):
                Wr = tf.get_variable("Wr", [self.embedding_size, self.embedding_size])
                Ur = tf.get_variable("Ur", [self.embedding_size, self.embedding_size])
                r = tf.sigmoid(tf.matmul(x, Wr) + tf.matmul(h, Ur), "r")
                Wz = tf.get_variable("Wz", [self.embedding_size, self.embedding_size])
                Uz = tf.get_variable("Uz", [self.embedding_size, self.embedding_size])
                z = tf.sigmoid(tf.matmul(x, Wz) + tf.matmul(h, Uz), "z")
                W = tf.get_variable("W", [self.embedding_size, self.embedding_size])
                U = tf.get_variable("U", [self.embedding_size, self.embedding_size])
                h_ = tf.tanh(tf.matmul(x,W)+tf.matmul(r*h,U))
                y = z*h + (1-z)*h_
            return y

        user_gated = GRU( user_embedding, user_nsvd_embedding, "User_gating")
        item_gated = GRU( item_embedding, item_nsvd_embedding, "Item_gating")
        # user_gating = tf.contrib.slim.fully_connected(tf.expand_dims(user_ratings_nonzero, 1), 50)
        # user_gating = tf.contrib.slim.fully_connected(user_gating, 1, activation_fn=tf.nn.sigmoid)
        # item_gating = tf.contrib.slim.fully_connected(tf.expand_dims(item_ratings_nonzero, 1), 50)
        # item_gating = tf.contrib.slim.fully_connected(item_gating, 1, activation_fn=tf.nn.sigmoid)

        # user_gating = tf.Print(user_gating, [tf.reduce_min(user_gating, axis=0),
        #                                      tf.reduce_mean(user_gating, axis=0),
        #                                      tf.reduce_max(user_gating, axis=0)], message="This is user_gating: ")
        # gating on nsvd_embedding
        # user_nsvd_embedding = user_gating * user_nsvd_embedding
        # item_nsvd_embedding = item_gating * item_nsvd_embedding
        # user_embedding = (1 - user_gating) * user_embedding
        # item_embedding = (1 - item_gating) * item_embedding
        ...
        merge_embedding = tf.concat([user_gated,item_gated],
                                    axis=1, name="merge_embedding")
