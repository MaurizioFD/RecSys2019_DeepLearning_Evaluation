...
class Model:
    ...
    def predict(self):
        ...
        # merge_embedding_ui = tf.concat([user_embedding, item_embedding ],
        #                             axis=1, name="merge_embedding_ui")
        # merge_embedding_nsvd = tf.concat([user_nsvd_embedding, item_nsvd_embedding],
        #                                axis=1, name="merge_embedding_nsvd")
        ...
        for i in xrange(len(self.layers) - 1):
            x_1 = tf.contrib.slim.fully_connected(x_1, self.layers[i + 1])

        x = tf.concat([x_1, GMF_u, GMF_i], axis=1, name="concat_embedding")
