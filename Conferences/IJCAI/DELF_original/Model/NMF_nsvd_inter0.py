...
class Model:
    ...
    def predict(self):
        ...
        item_nsvd_embedding = tf.expand_dims(tf.reciprocal(tf.sqrt(item_ratings_nonzero+1)),
                                             axis=1) * tf.matmul(item_ratings, embedding_users)
        def hbijk(embed_a, embed_b, variable_scope):
            with tf.variable_scope(variable_scope):
                W = tf.get_variable("W_0", [self.embedding_size, self.embedding_size],
                                    regularizer=regularizer)
                hb = tf.matmul(embed_a, W)*embed_b
            return hb
        hb_u0i0 = hbijk(user_embedding, item_embedding, "u0i0")
        hb_u0i1 = hbijk(user_embedding, item_nsvd_embedding, "u0i1")
        hb_u1i0 = hbijk(user_nsvd_embedding, item_embedding, "u1i0")
        hb_u1i1 = hbijk(user_nsvd_embedding, item_nsvd_embedding, "u1i1")
        ...
        #                              item_nsvd_embedding, ],
        #                             axis=1, name="merge_embedding")
        merge_embedding = tf.concat([hb_u0i0, hb_u0i1,
                                     hb_u1i0, hb_u1i1],
                                    axis=1, name="merge_embedding")
