...
def evaluate_model(model, testRatings, testNegatives, K, num_thread, sess, input_user, input_item, rating_matrix, train):
    ...
    batch_len=_model.batch_len//2
    batch_size = len(_testRatings)//batch_len
...
def eval_batch_rating(idx, batch_len, batch_size, batch_len_last):
    ...
    for i in xrange(len_items+1):
        predictions_this  = sess.run(_model.predict,
                               feed_dict={input_user: np.expand_dims(
                                   users_rep[i*batch_len:(i+1)*batch_len], axis=1),#(batch_len*(len_items+1),1)
                                          input_item: np.expand_dims(
                                              items.flatten()[i*batch_len:(i+1)*batch_len], axis=1),
                                          rating_matrix: _train})
        if i==0:
            predictions=predictions_this
        else:
            predictions = np.append(predictions, predictions_this,axis=0)
