...
class UNeuRec():
    def __init__(self, tf_session, num_neurons, num_factors, dropout_percentage, learning_rate, regularization_rate, max_epochs,
                 batch_size, display_epoch=1, display_step=None, verbose=True):
    ...
    def fit(self, urm):
        ...
        if self.verbose: print('UNeuRec: init..')
        ...
        R = tf.constant(urm, dtype=tf.float32)

        P = tf.Variable(tf.random_normal([self.num_items, hidden_dim_5], stddev=0.005))

        self.item_factor = tf.nn.embedding_lookup(P, self.cf_item_input)

        user_factor = tf.cond(self.isTrain, lambda: tf.nn.dropout(tf.nn.embedding_lookup(R, self.cf_user_input),1-self.dropout_percentage),
                              lambda: tf.nn.embedding_lookup(R, self.cf_user_input))

        W1 = tf.Variable(tf.random_normal([self.num_items, hidden_dim_1]))
        ...
        layer_1 = tf.sigmoid(tf.matmul(user_factor, W1) + b1)
        ...
        self.pred_y = tf.reduce_sum(tf.multiply(self.item_factor, self.layer_5), 1)

        self.loss = tf.reduce_sum(tf.square(self.y - self.pred_y)) + self.reg_rate * (
        ...
        if self.verbose: print('UNeuRec: init done')
    ...
    #         _ = self.run_epoch()
    #     if self.verbose: print("UNeuRec: training completed")

    def get_factors(self):

        learned_user_factors = self.sess.run(self.layer_5,
                                                  feed_dict={self.cf_user_input: np.arange(self.num_users),
                                                             self.isTrain: False})
        learned_item_factors = self.sess.run(self.item_factor,
