...
class ConvNCF_RecommenderWrapper(BaseRecommender, Incremental_Training_Early_Stopping, BaseTempFolder):
    ...
    #
    #
    # def _compute_score_ConvNCF_sparse(self, user_id_array, items_to_compute=None):
    #     n_users = len(user_id_array)
    #
    #     eurm = sps.csr_matrix(([], ([], [])), shape=(n_users, self.dataset.num_items))
    #
    #     for ui in range(n_users):
    #         predictions = np.array(ConvNCF.compute_score_single_user(user_id_array[ui],items_to_compute))
    #         row = np.full(predictions.shape[0],user_id_array[ui])
    #         col = items_to_compute
    #         data = predictions
    #         eurm += sps.csr_matrix((data,(row,col)), shape=(n_users, self.dataset.num_items))
    #
    #     return eurm
    ...
    def fit(self,
        ...
        if load_pretrained_MFBPR_if_available and os.path.isfile(self.args.path_partial_results + "best_model_latent_factors.npy"):
