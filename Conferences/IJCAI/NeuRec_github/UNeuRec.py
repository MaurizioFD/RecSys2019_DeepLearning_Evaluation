...
class UNeuRec():
    ...
    def run(self, train_data, train_user_item_matrix, unique_users, neg_train_matrix, test_matrix):
        ...
        item_factor = tf.nn.embedding_lookup(P, self.cf_item_input)

        user_factor = tf.cond(isTrain, lambda: tf.nn.dropout(tf.nn.embedding_lookup(R, self.cf_user_input), 1.00),
        ...
        for epoch in range(self.epochs):
            ...
            if (epoch) % 1 == 0 and epoch >= 0:
                ...
                learned_user_factors = self.sess.run(layer_5, feed_dict={self.cf_user_input: np.arange(self.num_users),
                                                                         isTrain: False})
