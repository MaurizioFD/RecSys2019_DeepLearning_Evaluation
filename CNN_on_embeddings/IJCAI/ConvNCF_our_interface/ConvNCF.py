...
# prediction model
class ConvNCF:
    def __init__(self, num_users, num_items, args, map_mode = "all_map"):
    ...
    def _create_inference(self, item_input):
        with tf.name_scope("inference"):
            ...
            if self._map_mode == "main_diagonal":
                print("ConvNCF: Using main diagonal elements.")
            ...
            elif self._map_mode == "off_diagonal":
                print("ConvNCF: Using off diagonal elements.")
                self.relation = tf.linalg.set_diag(self.relation, tf.zeros_like(tf.linalg.diag_part(self.relation)))

            else:
                print("ConvNCF: Using all map elements.")
...
def init_logging(args):
    regs = args.regs
    path = args.path_logging + "%s/" % (strftime('%d-%m-%Y', localtime()))
