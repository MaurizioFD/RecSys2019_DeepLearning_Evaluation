...
class FM(BaseEstimator, TransformerMixin):
    def __init__(self, data, user_field_M, item_field_M, pretrain_flag, save_file, hidden_factor, loss_type,
                    epoch, batch_size, learning_rate, lamda_bilinear, keep, optimizer_type, batch_norm,
                    verbose, permutation:list=None, random_seed=2016):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            self.permutation_matrix = tf.placeholder(tf.float32, shape=[None, None], name='permutation_placeholder')
            ...
            self.user_feature_embeddings = tf.nn.embedding_lookup(self.weights['user_feature_embeddings'],
                                                                  self.user_features)
            if self.permutation is not None:
                self.user_feature_embeddings = tf.tensordot(self.user_feature_embeddings, self.permutation_matrix, [2,1])

            self.positive_feature_embeddings = tf.nn.embedding_lookup(self.weights['item_feature_embeddings'],
                                                                     self.positive_features)
            if self.permutation is not None:
                self.positive_feature_embeddings = tf.tensordot(self.positive_feature_embeddings, self.permutation_matrix, [2,1])
            ...
            if self.permutation is not None:
                self.negative_feature_embeddings = tf.tensordot(self.negative_feature_embeddings, self.permutation_matrix, [2,1])
    ...
    def get_scores_per_user(self, user_feature):
        all_items_count = self.data.item_bind_M     #len(self.data.binded_items)

        X_user = [user_feature] * all_items_count
        X_item = self.data.all_items_features_list
        ...
        scores = scores.reshape(all_items_count)
        return scores
...
if __name__ == '__main__':
    ...
    fh = logging.FileHandler('../../../result_experiments/fm.log')
    ...
    save_file = '../../../result_experiments/pretrain-FM-%s/%s_%d' % (args.dataset, args.dataset, args.hidden_factor)
    # Training
    t1 = time()
